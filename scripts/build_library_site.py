...
<li class="book-card">
  <a href="./books/{book['slug']}/index.html" class="book-link">
    <span class="eyebrow">{html.escape(book['author'])}</span>
    <h2>{html.escape(book['title'])}</h2>
    <p>{html.escape(book['subtitle'])}</p>
    <p class="meta">{book['chapter_count']} chapters · bilingual digest + quiz</p>
  </a>
</li>
""".strip()
        )
  ...
  <body>
    <main class="container">
      <header class="hero">
        ...
        <h1>Choose a book</h1>
        <p class="subtitle">Bilingual chapter digests and quizzes collected in one site.</p>
      </header>
      <section class="card">
        <ul class="book-grid">
          {"".join(cards)}
